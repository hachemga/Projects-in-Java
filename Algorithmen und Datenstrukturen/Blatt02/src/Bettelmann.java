...
public class Bettelmann {
    ...
    public void playRound() {
        ...
        Deque<Card> openPile2 = new LinkedList<>();
            ...
            openPile1.addFirst(card1);
            openPile2.addFirst(card2);
            while ((card1.compareTo(card2) == 0) && (closedPile1.size() != 0) && (closedPile2.size() != 0)){
            ...
            if (card1.compareTo(card2) > 0 )  {
                    while (openPile1.size() != 0) {
                        closedPile1.addLast(openPile1.pollLast());
                    }
                    while (openPile2.size() != 0) {
                        closedPile1.addLast(openPile2.pollLast());
                    }
            }else if (card1.compareTo(card2) < 0)  { //|| (closedPile1.size() == 0))
                    while (openPile2.size() != 0) {
                        closedPile2.addLast(openPile2.pollLast());
                    }
                    while (openPile1.size() != 0) {
                        closedPile2.addLast(openPile1.pollLast());
                    }
            }

        if ((closedPile2.size() == 0) && (closedPile1.size() == 0)){
            winner = 0;
            return;
