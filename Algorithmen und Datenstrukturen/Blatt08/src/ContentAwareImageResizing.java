...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        ...
        ShortestPathsTopological topolo = new ShortestPathsTopological(G,sx*sy);
        Stack<Integer> direction = topolo.pathTo(sx*sy + 1);
        int[] p = new int[direction.size()-2];
        direction.remove(direction.size()-1);
        direction.remove(0);
        int index = 0;
        while (direction.size() != 0){
                p[index] = direction.pop() % sx;
                index++;
        }
        return p;
    }
